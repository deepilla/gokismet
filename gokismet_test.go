...
func compareStringMap(things string) func(exp, got map[string]string) []error {
	return func(exp, got map[string]string) []error {

		if len(got) != len(exp) {
			return sliceErrorf("Expected %d %s, got %d", len(exp), things, len(got))
		}
...
func compareRequestInfo(exp, got *RequestInfo) []error {
	...
	compareHeader := compareStringMap("request header(s)")
	errors = append(errors, compareHeader(exp.HeaderItems, got.HeaderItems)...)
...
func compareError(exp, got error) []error {
	...
	case nil:
		if got != nil {
			return sliceErrorf("Expected nil error, got %T %s", got, got)
		}
		return nil
	...
	case *gokismet.KeyError:
		...
		if !ok {
			return sliceErrorf("Expected a KeyError, got %T %s", got, got)
		}
	...
	case *gokismet.ValError:
		...
		if !ok {
			return sliceErrorf("Expected a ValError, got %T %s", got, got)
		}
		return compareValError(exp, err)

	default:
		if isErrorString(exp) {
			if !isErrorString(got) {
				return sliceErrorf("Expected an errorString, got %T %s", got, got)
			}
			return compareErrorString(exp, got)
		}
	}

	return sliceErrorf("Unhandled error type %T", exp)
}
...
func isErrorString(err error) bool {
	return fmt.Sprintf("%T", err) == "*errors.errorString"
}

func compareErrorString(exp, got error) []error {

	if got.Error() != exp.Error() {
		return sliceErrorf("Expected errorString %q, got %q", exp, got)
	}
...
func sliceErrorf(format string, args ...interface{}) []error {
	return sliceError(fmt.Errorf(format, args...))
}
