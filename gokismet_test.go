...
// Populate copies the values from an HTTP Request.
// NOTE: This function is not repeatable.
func (info *RequestInfo) Populate(req *http.Request) error {
...
// TestClient satisfies the gokismet Client interface. Its
// Do method captures incoming HTTP Requests and returns
// HTTP Responses defined by the caller. There are no
...
// NewVerifyingTestClient is a helper function that returns
// a TestClient set up to pass APIVerifyKey calls.
...
// Do extracts and stores information about the current HTTP
// Request and returns an HTTP Response defined by the caller.
func (c *TestClient) Do(req *http.Request) (*http.Response, error) {

	info, err := c.addRequest(req)
	if err != nil {
		return nil, err
	}
...
// ResetRequests deletes any HTTP Request information stored
// by the TestClient.
func (c *TestClient) ResetRequests() {
	c.Requests = c.Requests[:0]
}

// addRequest creates a RequestInfo object from the given
// HTTP Request and adds it to the RequestInfo array.
func (c *TestClient) addRequest(req *http.Request) (*RequestInfo, error) {

	info := &RequestInfo{}
	if err := info.Populate(req); err != nil {
		return nil, err
	}
...
// respond extracts the Akismet command from a request URL
// and returns the corresponding HTTP Response as defined
// by the caller.
func (c *TestClient) respond(curl string) (*http.Response, error) {

	// URLs are of the form https://rest.akismet.com/1.1/verify-key.
	// Base returns the last part of the URL, i.e. the API command.
	cmd := path.Base(curl)
...
// A FieldSetter uses reflection to set field values on
// any struct.
type FieldSetter struct {
	val reflect.Value
}

// NewFieldSetter returns a new FieldSetter ready to set
// fields on the provided struct. The caller must provide
// a pointer to a struct, otherwise its fields will not
// be settable via reflection.
...
var CommentData = []struct {
	...
	{
		// API includes the verified website by default.
	...
	},
	{
		...
		EncodedValues: "blog=http%3A%2F%2Fwww.example.com&comment_post_modified_gmt=2016-03-31T18%3A27%3A59Z&comment_type=comment&permalink=http%3A%2F%2Fexample.com%2Fposts%2Fthis-is-a-post%2F&referrer=http%3A%2F%2Fwww.google.com&user_agent=Mozilla%2F5.0+%28X11%3B+Linux+x86_64%29+AppleWebKit%2F537.36+%28KHTML%2C+like+Gecko%29+Chrome%2F41.0.2227.0+Safari%2F537.36&user_ip=127.0.0.1",
	},
	{
		Field: "Author",
		...
		EncodedValues: "blog=http%3A%2F%2Fwww.example.com&comment_author=Funny+commenter+name&comment_post_modified_gmt=2016-03-31T18%3A27%3A59Z&comment_type=comment&permalink=http%3A%2F%2Fexample.com%2Fposts%2Fthis-is-a-post%2F&referrer=http%3A%2F%2Fwww.google.com&user_agent=Mozilla%2F5.0+%28X11%3B+Linux+x86_64%29+AppleWebKit%2F537.36+%28KHTML%2C+like+Gecko%29+Chrome%2F41.0.2227.0+Safari%2F537.36&user_ip=127.0.0.1",
	},
	{
		Field: "AuthorEmail",
	...
	},
	{
		...
		EncodedValues: "blog=http%3A%2F%2Fwww.example.com&comment_author=Funny+commenter+name&comment_author_email=first.last%40gmail.com&comment_author_url=http%3A%2F%2Fblog.domain.com&comment_content=%3Cp%3EThis+blog+comment+contains+%3Cstrong%3Ebold%3C%2Fstrong%3E+and+%3Cem%3Eitalic%3C%2Fem%3E+text.%3C%2Fp%3E&comment_post_modified_gmt=2016-03-31T18%3A27%3A59Z&comment_type=comment&permalink=http%3A%2F%2Fexample.com%2Fposts%2Fthis-is-a-post%2F&referrer=http%3A%2F%2Fwww.google.com&user_agent=Mozilla%2F5.0+%28X11%3B+Linux+x86_64%29+AppleWebKit%2F537.36+%28KHTML%2C+like+Gecko%29+Chrome%2F41.0.2227.0+Safari%2F537.36&user_ip=127.0.0.1",
	},
	{
		Field: "Timestamp",
		Value: time.Date(2016, time.April, 1, 9, 0, 0, 0, time.UTC),
		Values: map[string]string{
			"user_ip":                   "127.0.0.1",
			"user_agent":                "Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/41.0.2227.0 Safari/537.36",
			"referrer":                  "http://www.google.com",
...
func testResponse(t *testing.T, fn StatusErrorFunc, moredata []TestResponseData) {
	...
	// Test scenatios for all API calls.
	data := []TestResponseData{
		{
